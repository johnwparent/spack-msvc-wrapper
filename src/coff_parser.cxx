...
enum : std::uint16_t { ValidCoffSiG2 = 0xFFFF, MemberNameLen = 16 };
...
void CoffParser::ParseShortImport(coff_member* member) {
    ...
    // validate header
    if (!(im_h->Sig1 == 0x00) || !(im_h->Sig2 == ValidCoffSiG2)) {
        return;
    }
...
void CoffParser::ParseFullImport(coff_member* member) {
    ...
    for (DWORD i = 0; i < file_h->NumberOfSymbols; ++i) {
...
bool CoffParser::ParseData(PIMAGE_ARCHIVE_MEMBER_HEADER header,
    ...
    if ((p_imp_header->Sig1 == IMAGE_FILE_MACHINE_UNKNOWN) &&
        ...
        CoffParser::ParseShortImport(member);
                        ...
                        IMAGE_ARCHIVE_LINKER_MEMBER, MemberNameLen)) {
        ...
        if (!this->coff_.read_first_linker) {
            ...
            CoffParser::ParseSecondLinkerMember(member);
                        ...
                        IMAGE_ARCHIVE_LONGNAMES_MEMBER, MemberNameLen)) {
...
void CoffParser::NormalizeLinkerMember(const std::string& name,
                                       ...
                                       const char* strings,
                                       const DWORD symbols) {
    size_t const offset_with_header =
        base_offset + sizeof(IMAGE_ARCHIVE_MEMBER_HEADER);
    size_t current_relative_offset = 0;
    for (DWORD j = 0; j < symbols; ++j) {
        size_t const name_len = strlen(strings + current_relative_offset);
        std::unique_ptr<char[]> new_name(new char[name_len + 1]);
        strncpy(new_name.get(), strings + current_relative_offset, name_len);
        new_name[name_len] = '\0';
        if (strstr(new_name.get(), name.c_str())) {
            replace_special_characters(new_name.get(), name_len);
            ...
            this->coffStream_->write(new_name.get(), name_len);
        }
...
void CoffParser::NormalizeSectionNames(const std::string& name, char* section,
    ...
    while (section_search_start && (section_search_start < search_terminator)) {
        ...
        if (section_search_start) {
            ...
            std::unique_ptr<char[]> new_name(new char[name_len + 1]);
            strncpy(new_name.get(), section_search_start, name_len);
            new_name[name_len] = '\0';
            replace_special_characters(new_name.get(), name_len);
            this->writeRename(new_name.get(), name_len,
...
bool CoffParser::NormalizeName(std::string& name) {
    ...
    for (auto mem : this->coff_.members) {
        ...
        while (pos < MemberNameLen && mem.header->Name[pos] != ' ') {
            ++pos;
        }
        std::string const name_ref =
            std::string(reinterpret_cast<char*>(mem.header->Name), pos);
        if (!endswith(name_ref, "/")) {
            // We have an import member
            // Name is longer than MemberNameLen bytes, need to lookup name in longname offset
            ...
            if (this->coff_.members.size() <= 2 ||
                !this->coff_.members[2].member ||
                !this->coff_.members[2].member->data) {
                continue;
            }
            ...
            std::unique_ptr<char[]> long_name(new char[long_name_len + 1]);
            strncpy(long_name.get(),
                    this->coff_.members[2].member->data + longname_offset,
                    long_name_len);
            long_name[long_name_len] = '\0';
            if (CoffParser::matchesName(long_name.get(), name) &&
                !long_name_renamed) {
                // If so, unmangle it
                replace_special_characters(long_name.get(), long_name_len + 1);
                // offset of actual longname member
                size_t const offset =
                    std::streamoff(this->coff_.members[2].offset);
                this->writeRename(long_name.get(), long_name_len + 1,
            ...
            if (mem.member->is_short) {
                ...
                std::unique_ptr<char[]> new_name(new char[name_len + 1]);
                // unmangle it
                strncpy(new_name.get(), mem.member->short_member->short_dll, name_len);
                new_name[name_len] = '\0';
                replace_special_characters(new_name.get(), name_len);
                ...
                if (CoffParser::matchesName(mem.member->short_member->short_dll,
                    ...
                    size_t offset = std::streamoff(mem.offset);
                    ...
                    this->writeRename(new_name.get(), strlen(new_name.get()), offset);
                }
            } else {
                ...
                for (int j = 0; j < section_data_count; ++j) {
                    ...
                    DWORD data_size = psec_header->SizeOfRawData;
                    DWORD const virtual_size = psec_header->Misc.VirtualSize;
                ...
                size_t const relative_string_table_start_offset =
                    std::streamoff(mem.offset) +
                    sizeof(IMAGE_ARCHIVE_MEMBER_HEADER) +
                ...
                for (DWORD j = 0; j < symbol_count; ++j) {
                    ...
                    if (symbol->N.Name.Short == 0) {
                        ...
                        if (string_table_start &&
                            ...
                            std::unique_ptr<char[]> new_no_ext_name(new char[name_len + 1]);
                            strncpy(new_no_ext_name.get(), string_table_start,
                                    name_len);
                            new_no_ext_name[name_len] = '\0';
                            replace_special_characters(new_no_ext_name.get(),
                                                       name_len);
                            this->writeRename(
                                new_no_ext_name.get(), name_len,
            ...
            size_t const base_offset = std::streamoff(mem.offset);
